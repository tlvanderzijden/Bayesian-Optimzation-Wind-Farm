function [hypStruct, logp] = tuneHyperparametersStruct(Xm, fmh, hyp)
	...
	for j = 1:maxReductions
		...
		if min(newHyp > 0) % The parameters have to remain positive. If they are not, something is wrong. To be precise, the step size is too big.
			...
			if ~exist('logp','var') || newLogp >= logp
				...
				newHypDeriv(2) = 1/(2*lf2^2)*trace(R*Kmm);
				for k = 1:dx
					newHypDeriv(2+k) = 1/(4*lx2(k)^4)*trace(R*(Kmm.*(diff(:,:,k).^2)));
				end
...
hypStruct.cov = zeros(dx+1,1);
% We extract all the parameters.
hypStruct.lik = log(sqrt(hyp(1)));
hypStruct.cov(end) = log(sqrt(hyp(2)));
hypStruct.cov(1:dx) = log(sqrt(hyp(3:end)));
